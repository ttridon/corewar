...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/03/06 18:00:24 by ttridon           #+#    #+#             */
/*   Updated: 2017/04/05 16:24:47 by ttridon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "vm.h"

int		cw_ld(unsigned char *arena, t_process *process, t_game *game)
{
	int		params[MAX_ARGS_NUMBER];
	int		oct_codage;

	if (cooldown(process, LD_COOLDOWN) && game)
		return (0);
	...
	if ((FIRST_OCT != 0 && FIRST_OCT != REG_CODE && SECOND_OCT == REG_CODE))
	{
		if (!get_params(arena, process->pc + 1, DIR_SIZE, params))
		{
			if (FIRST_OCT == IND_CODE)
				params[0] = get_value(arena,
					(process->pc + params[0]) % IDX_MOD, 4);
