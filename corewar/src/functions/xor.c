...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/03/07 12:05:33 by ttridon           #+#    #+#             */
/*   Updated: 2017/04/05 17:05:25 by ttridon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "vm.h"

static void	cw_xor_bis(int oct_codage, int *params,
...
int			cw_xor(unsigned char *arena, t_process *process, t_game *game)
{
	int		params[MAX_ARGS_NUMBER];
	int		oct_codage;

	if (cooldown(process, XOR_COOLDOWN) && game)
		return (0);
	...
	if ((FIRST_OCT != 0 && SECOND_OCT != 0 && THIRD_OCT == REG_CODE))
	{
		if (!get_params(arena, process->pc + 1, DIR_SIZE, params))
			cw_xor_bis(oct_codage, params, process, arena);
		else
			process->carry = 0;
	}
	else
