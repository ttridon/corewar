...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/03/09 17:29:25 by ttridon           #+#    #+#             */
/*   Updated: 2017/04/05 17:00:54 by ttridon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "vm.h"

static void	cw_lldi_bis(int oct_codage, int *params,
...
int			cw_lldi(unsigned char *arena, t_process *process, t_game *game)
{
	int		params[MAX_ARGS_NUMBER];
	int		oct_codage;

	if (cooldown(process, LLDI_COOLDOWN) && game)
		return (0);
		...
		&& SECOND_OCT != IND_CODE) && THIRD_OCT == REG_CODE)
	{
		if (!get_params(arena, process->pc + 1, DIR_SIZE / 2, params))
			cw_lldi_bis(oct_codage, params, process, arena);
		else
			process->carry = 0;
	}
	else
